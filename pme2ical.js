...
function getICalendarFeed(pmeResults, yEntityId, mode) {
	...
	var yEntity, task, vEvent, startDateTime, endDateTime, startDateTimeCET, endDateTimeCET, startDateString, subject, eventDuration;
	...
	for (var i=0; i<tasks.length; i++) {
		...
		if (task.RI === yEntityId) {
            //get event times
		    startDateTime = new Date(parseInt(task.SD.substr(6)) + tzOffset);
            endDateTime = new Date(parseInt(task.ED.substr(6)) + tzOffset);
